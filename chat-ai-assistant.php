...
class Chat_AI_Assistant {    
    ...
    public function enqueue_admin_styles() {
        wp_enqueue_style('wp-chat-admin-style', plugins_url('css/wp-chat-admin.css', __FILE__), array(), $this->plugin_version);
    }
    public function enqueue_admin_scripts() {
        wp_enqueue_script('wp-chat-admin-script', plugins_url('js/wp-chat-admin.js', __FILE__), array('jquery'), $this->plugin_version, true);
    }
    
    public function register_settings() {
        // Register settings group
        register_setting('chat-ai-assistant-settings-group', 'chat_enabled');
        register_setting('chat-ai-assistant-settings-group', 'chatgpt_api_key');
        register_setting('chat-ai-assistant-settings-group', 'chatgpt_api_model');
        register_setting('chat-ai-assistant-settings-group', 'chatgpt_assistant_name');
        register_setting( 'chat_ai_assistant_settings_group', 'chat_ai_assistant_option_name' );       
    ...
    public function add_admin_menu() {
        add_options_page('Chat AI Settings', 'Chat AI Assistant', 'manage_options', 'chat-ai-assistant-settings', array($this, 'render_settings_page'));
    }
    public function render_settings_page() {
        ?>
        <div class="wrap wp-chat-admin-wrap">
            <h2>Chat AI Settings</h2>
            
            <form method="post" action="options.php">
                <?php settings_fields('chat-ai-assistant-settings-group'); ?>
                <?php do_settings_sections('chat-ai-assistant-settings-group'); ?>
            ...
            ?>
                ...
                <?php wp_nonce_field( 'chat_ai_assistant_admin_action', 'chat_ai_assistant_admin_nonce' ); ?>
                <?php submit_button(); ?>
            </form>
        </div>
        <?php
    ...
    public function localize_ajaxurl() {
        wp_localize_script('chat-popup-script', 'ajax_object', array('ajaxurl' => admin_url('admin-ajax.php'),'nonce'=>wp_create_nonce( 'chat_ai_assistant_action' )));
    }

    public function process_chat_message() {
        if (isset( $_POST['chat_ai_assistant_nonce'] ) && wp_verify_nonce( $_POST['chat_ai_assistant_nonce'], 'chat_ai_assistant_action' ) ) {
        //echo $message;exit;
    ...
    public function enqueue_scripts() {
        ...
        if (get_option('chat_enabled')) {
            wp_enqueue_style('wp-chat-popup-style', plugins_url('css/wp-chat-popup.css', __FILE__), array(), $this->plugin_version);
            wp_enqueue_script('chat-popup-script', plugins_url('js/wp-chat-popup.js', __FILE__), array('jquery'), $this->plugin_version, true);
    ...
    private function custom_word_search($search_term) {
        ...
        $text_without_html = wp_strip_all_tags($content);
...
$chat_ai_assistant = new Chat_AI_Assistant();
